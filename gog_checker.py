#!/usr/bin/env python3
"""
GOG Price Checker - Standalone Script
This script can be run directly without installing the package.
It allows checking prices for individual games or entire wishlists across different countries.
"""
...
# Price class for storing country price information
class Price:
...
# Dictionary of countries and their names
COUNTRIES = {
    "US": "United States",
    "AR": "Argentina",
    "BS": "Bahamas",
...
def extract_gog_data(html_content):
    ...
    try:
        # Try different regex patterns to find the gogData object
        patterns = [
            r'window\.gogData\s*=\s*(\{.*?\});\s*(?:window\.|var)',
            r'var\s+gogData\s*=\s*(\{.*?\});',
            r'<script>\s*var\s+gogData\s*=\s*(\{.*?\});\s*</script>'
        ]
...
def init_parser():
    ...
    parser = ArgumentParser(description="GOG Price Checker - Check game prices across different countries")
    parser.add_argument("-u", "--url", type=str, help="URL of the game page to scrape")
    ...
    parser.add_argument("-c", "--count", type=int, default=10, help="Number of countries to show (default: 10)")
    parser.add_argument("-p", "--pretty", action="store_true", help="Show result as pretty table")
...
def main():
    ...
    if args.wishlist:
        ...
        print("Example: ./gog_checker.py -u https://www.gog.com/game/diablo -n -p")
        print("Example: ./gog_checker.py -w username -p")
